...
2. **Crosswalk review: the structure is done, and my legacy tag review is in progress.**
   ...
   - I am now reviewing and reclassing legacy tags in OLM by hand, using a reference workbook (`review/legacy_tag_review.xlsx`, local and untracked, one row per tagged object). The REVIEW backlog is too large for me to work by hand, so Claude maintains it: after I tell you I've made OLM edits, run a fresh sync, rebuild the workbook, and report the remaining REVIEW items grouped by their "OLM data needs fixes" note, flagging inconsistencies for me. Save the generator script in the repo so rebuilds are reproducible (the original wasn't saved).
...
3. **Plan the code build (waiting on me).** Write a plan for me to approve before changing any code: the engine in `scripts/sync_data.py` that reads `config/crosswalk.csv` and applies the README's matching rules, the audit report, then the layer tree map in `index.html`. Proposed treatment of orphan custom tags (no parent object, usually typos): keep them off the map layers, and list them in the audit as their own "Orphan tags" line with photo IDs, separate from UNMAPPED. Confirm this in the plan. The README's "Current tree" was updated to follow the sheet on 2026-09-25.
